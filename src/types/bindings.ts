 ...
 */
liveClassPosition: number; startPosOverall: number; startPosClass: number; lap: number; lapDistPct: number; lastLapTime: number; bestLapTime: number; f2Time: number; estTime: number; trackSurface: TrackSurface; iRating: number; licString: string; licColor: string; incidents: number; isPlayer: boolean; onPitRoad: boolean; estimatedIrDelta: number | null; relativeLapDist: number; classEstLapTime: number; rawFlags: number; resultsPositionLap: number | null; resultsPositionTime: number | null; pitState: PitState }
